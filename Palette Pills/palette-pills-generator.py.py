...
def main():
    ...
    for mode in modes:
        ...
        for name, palette in PALETTES.items():
            ...
            # --- INTELLIGENT COLOR SWAPPING ---
            # For Monolith (Greyscale), if we are in Light Mode,
            # we invert the palette so the white bars don't blend into the light background.
            current_palette = palette.copy()
            if mode_name == "Light" and name == "Monolith":
                current_palette = list(reversed(palette))
                print(f"    (Smart adjustment applied to Monolith for Light Mode)")

            try:
                # Generate
                img = generate_design(w, h, current_palette, bg, super_sample=2)
    ...
    print("-" * 50)
    print("Done! Check the 'Palette_Pills' folder.")
